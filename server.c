...
#include <pthread.h>
#include <dispatch/dispatch.h>  // Include Grand Central Dispatch for semaphores

#define PORT 8080
#define MAX_CONCURRENT_CLIENTS 2

pthread_mutex_t log_mutex = PTHREAD_MUTEX_INITIALIZER;
dispatch_semaphore_t file_semaphore;
...
int main() {
    // Initialize the semaphore with MAX_CONCURRENT_CLIENTS
    file_semaphore = dispatch_semaphore_create(MAX_CONCURRENT_CLIENTS);
    ...
    dispatch_release(file_semaphore); // Release the semaphore after usage
    return 0;
}
...
void handle_upload(int clientsock, char* filename) {
    dispatch_semaphore_wait(file_semaphore, DISPATCH_TIME_FOREVER);
    FILE* fp = fopen(filename, "wb");
    if (!fp) {
        perror("Error opening file");
        send_reply_to_client(clientsock, "Error opening file");
        dispatch_semaphore_signal(file_semaphore);
        return;
    }
...
void handle_screenshot(int clientsock) {
    // Use macOS's screencapture to take a screenshot
    system("screencapture /tmp/screenshot.png");
